...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('r') and self.debug then
			self.board:initializeTiles()
		end
		
		if love.keyboard.wasPressed('e') and self.debug then
			self.timer = 0
		end
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
               ...
               local newTile = self.board.tiles[y][x]
				...
				self:swap(self.highlightedTile, newTile)
...
function PlayState:swap(tile1, tile2)
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
			...
			for k, match_tile in pairs(match) do
				score = score + match_tile.variety
			end
    ...
    -- if no matches, we can continue playing
        ...
        while not self.board:availableMoves() do
			self.board:initializeTiles()
		end
        self.canInput = true
    end
